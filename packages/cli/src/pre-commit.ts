...
const SHADSCAN_TOKEN_PATTERN =
  /(?:^|[\s"'`;&|()])(?:[^\s"'`;&|()]*(?:\/|\\))?shadscan(?:\.cmd)?(?=$|[\s"'`;&|()])|shadscan-svelte@[0-9A-Za-z.+-]+/;
const DIRECT_SHADSCAN_COMMAND_PATTERN =
  /^(?:(?:[^\s"'`;&|()]+(?:\/|\\))?shadscan(?:\.cmd)?|bunx\s+shadscan-svelte@[0-9A-Za-z.+-]+|pnpm\s+dlx\s+shadscan-svelte@[0-9A-Za-z.+-]+|npx\s+--yes\s+shadscan-svelte@[0-9A-Za-z.+-]+|yarn\s+dlx\s+--quiet\s+--package\s+shadscan-svelte@[0-9A-Za-z.+-]+\s+shadscan-svelte)(?=\s|$)/u;
...
const createAuditCommand = ({
  ...
  const packageSpecifier = `shadscan-svelte@${version}`;
...
const createPreCommitInstallPlan = async (
  ...
  if (options.packageManager === "unknown") {
    return createManualPlan({
      ...
      steps: [
        `Add shadscan-svelte@${options.version} with the repository's package manager.`,
...
const formatPreCommitInstallPlan = (plan: PreCommitInstallPlan): string => {
  const lines = [
    ...
    `  CLI: shadscan-svelte@${plan.version}`,
